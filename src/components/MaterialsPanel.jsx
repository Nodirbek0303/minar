...
export default function MaterialsPanel({ project, onSaved, onUnauthorized }) {
  ...
  const printSpec = () => {
    ...
    '<table><thead><tr><th>Qavat</th><th>Mahsulot</th><th>Birlik</th><th>Miqdor</th>' +
      ...
      (priceCols ? '<tfoot><tr><td colspan="5">JAMI' + (rent ? ' (' + months + ' oylik arenda)' : '') + '</td><td>' + qty(shownTotal) + '</td></tr></tfoot>' : '') +
    '</table>' +
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontWeight: 700, color: 'var(--accent)', fontSize: 16 }}>
            {showPrices ? som(shownTotal) : rows.length + ' mahsulot'}
          </div>
          <div className="small-muted">
            {rows.length} pozitsiya · qolip {q.facadeArea || 0} m² · {project.schedule?.totalDays} kun montaj
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.key}>
                ...
                {showPrices && (
                  <td className="num" onClick={() => setEdit({ key: r.key, value: String(r.matRate) })} style={{ cursor: 'pointer' }} title="Narxni o'zgartirish uchun bosing">
                    {edit?.key === r.key ? (
                      <input
            ...
            <tfoot>
              <tr>
                <td colSpan={floorFilter === 'all' ? 5 : 4} style={{ textAlign: 'right', fontWeight: 700 }}>
                  JAMI{rent ? ` (${months} oylik arenda)` : ''}
                </td>
                <td />
                <td className="num" style={{ fontWeight: 800, color: 'var(--accent)' }}>{qty(shownTotal)}</td>
              </tr>
            </tfoot>
          )}
        </table>
